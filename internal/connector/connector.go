...
// PostgreSQL driver
_ "github.com/lib/pq"
// SQL Server driver (official Microsoft Go driver — github.com/microsoft/go-mssqldb)
...
User      string
Password  string
// SQL Server TLS options (ignored for other drivers)
Encrypt   bool
TrustCert bool
}
...
if strings.EqualFold(c.Driver, "sqlite") || strings.EqualFold(c.Driver, "sqlite3") {
...
// Credentials are embedded where the driver requires it; they are NEVER logged.
...
// Build the DSN using url.URL so that special characters in
// user/password are safely percent-encoded.
u := &url.URL{
Scheme: "sqlserver",
...
q.Set("encrypt", fmt.Sprintf("%t", c.Encrypt))
q.Set("TrustServerCertificate", fmt.Sprintf("%t", c.TrustCert))
u.RawQuery = q.Encode()
return u.String(), nil
case "sqlite", "sqlite3":
return c.Database, nil
default:
return "", fmt.Errorf("unsupported driver %q; supported: sqlserver, mysql, postgres, sqlite", c.Driver)
}
}
...
// RunQuery connects to the database, executes the query, and returns a JSON-encoded result.
